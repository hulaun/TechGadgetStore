...
export default function CreateAccount() {
    ...
    function handleResetPress() {
        ...
        router.replace("/(auth)/sign-up/VerifyCode");
    }
    ...
    return (
        <KeyboardDismissWrapper className="flex-1 justify-between flex-col" style={{backgroundColor: theme.colors.background}}>
            ...
            <View className="mb-4 justify-center px-8">
                <Text className="text-3xl font-bold mb-2" style={{color: theme.colors.text}}>Register Account</Text>
                <Text className="text-gray-500 mb-12">Masukan Email/ No. Hp untuk mendaftar </Text>
                ...
                <TouchableOpacity
                    className={`w-full p-4 mt-8 rounded-lg ${!errors.email && email ? 'bg-blue-500' : 'bg-gray-500'}`}
            ...
            {!isKeyboardVisible ? (
                <View className="mb-10 w-full flex-row px-8 justify-center">
                    <TouchableOpacity>
                        <Text>Already have an account? <Link href={'/LoginScreen'} className={'text-blue-500'}>Log in</Link></Text>
                    </TouchableOpacity>
                </View>
            ) : <View />}
